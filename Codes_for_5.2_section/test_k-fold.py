...
X_train, X_test, y_train, y_test = train_test_split(
    features_np, label_idx_np, test_size=0.1, random_state=42
)
print(len(X_train))
print(len(X_test))
# print(y_test)
...
shape_conv1_w = math.floor((IN_FEAT+2*10-(20-1)-1)/1)+1         #  in torch.nn.conv1d()and nn.cov2d, stride default is 1, dilation default is 1
shape_conv1_h = math.floor((1+2*10-(20-1)-1)/1)+1
shape_maxpool2d_w = math.floor((shape_conv1_w+2*1-(2-1)-1)/2)+1    # in torch.nn.MaxPool2d(), stride default is kernel size, dilation default is 1, padding default is 0
shape_maxpool2d_h = math.floor((shape_conv1_h+2*1-(2-1)-1)/2)+1
...
class QuantizedConv1DModel(nn.Module):
    def __init__(self, input_size, num_classes, bit_width=8):
        ...
        self.pool1 = qnn.QuantMaxPool2d(kernel_size=(2,2),padding=1)
...
def train(
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train)):
        ...
        # print(f"X_train_fold shape: {X_train_fold.shape} | type: {type(X_train_fold)}")
        # print(f"y_train_fold shape: {y_train_fold.shape} | type: {type(y_train_fold)}")
        # print(f"X_val_fold shape: {X_val_fold.shape} | type: {type(X_val_fold)}")
        # print(f"y_val_fold shape: {y_val_fold.shape} | type: {type(y_val_fold)}")
        
        train_loader = DataLoader(
             TensorDataset(X_train_fold, y_train_fold), batch_size=batch_size, shuffle=shuffle
        )
        # val_loader = DataLoader(
             # TensorDataset(X_val_fold, y_val_fold), batch_size=batch_size, shuffle=shuffle
        # )
        #torch_model.train()
        for epoch in range(epochs):
             ...
             y_true_all = []
             torch_model.train()
             #print(f"y_pred_all type: {type(y_pred_all)}")
             for batch_index, (X_batch, y_batch) in enumerate(train_loader):
                 ...
                 #np.append(y_pred_all, y_pred.argmax(1).detach().cpu().numpy())
             ...
             #print(f"y_pred_all type: {type(y_pred_all)} | y_true_all type: {type(y_true_all)}")
             accuracy = numpy.mean(y_pred_all == y_true_all)
             #print(f"accuracy of epoch {epoch}: {accuracy}")

             # Validation phase for the current epoch
             torch_model.eval()
             val_fp32_pred = torch_model(X_val_fold.to(device)).cpu().argmax(1).float().detach().numpy()
             # print(f"val_fp32_pred type: {type(val_fp32_pred)} | val_fp32_pred shape: {val_fp32_pred.shape}")
             # print(f"y_val_fold type: {type(y_val_fold)} | y_val_fold shape: {y_val_fold.shape}")
             
             val_accuracy = numpy.mean(val_fp32_pred == y_val_fold.detach().cpu().numpy())     # val_fp32_pred type is numpy, but y_val_fold type is tensor. So change tensor to numpy by *.numpy() 
             print(
