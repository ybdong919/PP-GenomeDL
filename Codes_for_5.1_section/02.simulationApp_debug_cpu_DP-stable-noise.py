...
def calibrateAnalyticGaussianMechanism(epsilon, delta, GS=1, tol=1.e-12):
    ...
    """
        ...
        sigma : standard deviation of Gaussian noise needed to achieve (epsilon,delta)-DP under global sensitivity GS
    """
    ###### set scale parameter (c)
    sigma = GS*sqrt(np.log(2/delta)) / epsilon
    return sigma
...
def dp(grad, device,epsilon=8, delta=1e-3):
    ...
    #########(1,2) The two DP noise algorithms: the analytic Gaussian mechanism and classic Gaussian mechanism
    #result = torch.tensor(np.random.normal(0, sigma, grad.shape), device=device) + grad
    
    ########################## (3) The DP noise algorithm: Correlated Gaussian mechanism [Lebeda, Better gaussian mechanism using correlated noise, 2025]  #############################################
    ...
    ########################## (5) The fifth DP noise algorithm: levy-stable mechanism  ################################################################# 
    #################### the stable distribtion has 4 parameters: alpha (characteristic exponent, 0<a<=2; when a=2, normal distribution), beta=0 (skewness parameter, [-1,1], When it is zero, the distribution is symmetrical), u=0 (location parameter), c=sigma (scale parameter)#############
    alpha = 2*(l2_norm_clip/epsilon)/((l2_norm_clip/epsilon)+1)
    #alpha = 0.5
    beta = 0
    loc = 0
    c = 1
    result = torch.tensor(levy_stable.rvs(alpha, beta, loc, scale=c, size=grad.shape), device=device) + grad
