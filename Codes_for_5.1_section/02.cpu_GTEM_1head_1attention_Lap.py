...
def initialClientModel(serverModel,device):
    ...
    #clientModel=Model(batch_size=BATCH_SIZE,n_head=5,n_gene=20531,n_feature=20531,n_class=len(cancerTypes),d_ff=1024,dropout_rate=0.3)
    clientModel=Model(batch_size=BATCH_SIZE,n_head=1,n_gene=20531,n_feature=20531,n_class=len(cancerTypes),d_ff=1024,dropout_rate=0.3)
...
def calibrateAnalyticGaussianMechanism(epsilon, delta, GS=1, tol=1.e-12):
    ...
    ################### It is Laplace mechanism of [Holohan N. The bounded laplace mechanism in differential privacy]
    """
        Arguments:
        epsilon : target epsilon (epsilon > 0)
        delta : target delta (0 < delta < 1)
...
def dp(grad, device,epsilon=8, delta=1e-3):
    ...
    ########################## (4) The fourth DP noise algorithm: classic Laplace mechanism  #################################################################
    result = torch.tensor(np.random.laplace(0, sigma, grad.shape), device=device) + grad
...
if __name__ == '__main__':
    ...
    # Model
    #serverModel = Model(input_size=20531,output_size=len(cancerTypes))  # original Model
    # d_ff = 1024
    # n_gene = 20531      # it is for model layers
    # n_feature = 20531   # it is for attention
    #serverModel = Model(batch_size=BATCH_SIZE,n_head=5,n_gene=20531,n_feature=20531,n_class=len(cancerTypes),d_ff=1024,dropout_rate=0.3).to(device)  ### use Transformer Model
    serverModel = Model(batch_size=BATCH_SIZE,n_head=1,n_gene=20531,n_feature=20531,n_class=len(cancerTypes),d_ff=1024,dropout_rate=0.3).to(device)  ### use Transformer Model
